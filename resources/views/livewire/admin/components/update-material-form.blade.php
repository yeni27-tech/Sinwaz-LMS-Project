            ...
            <section>
                ...
                @method('put')
                    ...
                    <div>
                        ...
                        <x-text-input wire:model="name" value="{{ $this -> materialById -> name }}" id="name" class="block mt-1 w-full" type="text" name="name" required autofocus autocomplete="name" />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="course_id" :value="__('Divisi')" />

                        <x-select name="course_id"  id="course_id" class=" w-full h-fit text-xs">
                                <x-option-select selected value="{{ $this -> materialById -> course_id }}">{{ $this -> materialById -> course->name }}</x-option-select>
                    ...
                    <div>
                        ...
                        <input wire:change="inputYTVideoLink($event.target.value)" name="yt_video_link" id="yt_video_link" class="w-full  p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Type quiz yt_video_link" value="{{ $this -> materialById -> yt_video_link }}" />
