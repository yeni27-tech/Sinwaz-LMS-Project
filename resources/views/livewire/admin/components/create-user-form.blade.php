{{--
 <div class=" bg-black/20 h-full w-screen fixed left-0 z-50 top-0 right-0 flex justify-center items-center">
        <div class=" relative w-11/12 h-5/6 p-4 overflow-scroll bg-slate-50 rounded-lg">
            <h1 class=" mb-4 text-xl font-bold ">Tambah Data User</h1>

            <div wire:click='closeCreateUserForm' class=" hover:cursor-pointer text-slate-900 absolute right-4 top-4">
                x
            </div> --}}

            <form action="{{ route('user.store') }}" method="post" class=" space-y-2 z-50 p-4">
                @csrf
                <div>
                ...
                <div class=" grid grid-cols-2 gap-2">
                    ...
                    <div>
                        ...
                        <x-select  name="role" class=" w-full h-fit text-xs">
